...
/*:
* @plugindesc v1.47 The smallest, fastest lighting plugin! - Galv Map Projectiles Support
...
Lightmask.prototype.reloadProjectileEvents = function () {
    //**********************fill up new map-array *************************
    this.projectile_note = [];
    this.projectile_id = [];
    this.projectile_stacknumber = [];
    this.projectile_x = [];
    this.projectile_y = [];
    this.projectile_count = $gameMap._mapProjectileSprites.length;
    for (let i = 0, n = this.projectile_count; i < n; i++) {
        try {
        let note = this.getTag.call($gameMap._mapProjectileSprites[i]);
        let note_args = note.split(" ");
        
        this.note_command = note_args.shift().toLowerCase();
        if (this.note_command == "light") {
            this.projectile_note.push(note);
            this.projectile_x.push($gameMap._mapProjectileSprites[i].x);
            this.projectile_y.push($gameMap._mapProjectileSprites[i].y);
            this.projectile_stacknumber.push(i);
        }}
        catch (e) {
            continue;
        }
